...
# modified by Carlos Kasprzykowski
# Aug 14 2017
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option("-k", "--apikey", dest="s_apikey", default=None,
                      help="Exchange IBM Xforce API key", metavar="apikey")
    parser.add_option("-p", "--apipw", dest="s_apipw", default=None,
                      help="Exchange IBM Xforce API password", metavar="apipw")
...
if ( options.s_apikey is not None) and ( options.s_apipw is not None):
    key = str(options.s_apikey) # https://exchange.xforce.ibmcloud.com/settings/api
    password = str(options.s_apipw)
    token = base64.b64encode(key + ":" + password)
    headers = {'Authorization': "Basic " + token, 'Accept': 'application/json'}
    url = "https://api.xforce.ibmcloud.com:443"
else:
    print "API Key and Password must be provided"
    sys,exit(0)
